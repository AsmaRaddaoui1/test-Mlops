...
def preprocess_data(X, y):
    ...
    X_processed["International plan"] = X_processed["International plan"].map(
        {"No": 0, "Yes": 1}
    )
    y_processed["International plan"] = y_processed["International plan"].map(
        {"No": 0, "Yes": 1}
    )

    X_processed["Voice mail plan"] = X_processed["Voice mail plan"].map(
        {"No": 0, "Yes": 1}
    )
    y_processed["Voice mail plan"] = y_processed["Voice mail plan"].map(
        {"No": 0, "Yes": 1}
    )
    ...
    encoded_states_df_X = pd.DataFrame(
        encoded_states_X, columns=encoder_state.get_feature_names_out(["State"])
    )
    encoded_states_df_y = pd.DataFrame(
        encoded_states_y, columns=encoder_state.get_feature_names_out(["State"])
    )

    encoded_area_df_X = pd.DataFrame(
        encoded_area_X, columns=encoder_area.get_feature_names_out(["Area code"])
    )
    encoded_area_df_y = pd.DataFrame(
        encoded_area_y, columns=encoder_area.get_feature_names_out(["Area code"])
    )
    ...
    X_processed = pd.concat(
        [X_processed, encoded_states_df_X, encoded_area_df_X], axis=1
    )
    y_processed = pd.concat(
        [y_processed, encoded_states_df_y, encoded_area_df_y], axis=1
    )

    encoders = {"state_encoder": encoder_state, "area_encoder": encoder_area}
...
def prepare_training_data(X, y, sampling_strategy=0.3):
    ...
    print(
        f"   - Après rééchantillonnage: {pd.Series(y_resampled).value_counts().to_dict()}"
    )
...
def evaluate_model(model, X_test, y_test, model_name=""):
    ...
    if hasattr(model, "predict_proba"):
        ...
        metrics_dict.update({"roc_auc": roc_auc, "log_loss": logloss})
...
def load_saved_model(filepath):
    ...
    try:
        ...
        if bundle is None:
            ...
            for ext in (".pka", ".joblib", ".pkl"):
                path_with_ext = (
                    filepath if filepath.lower().endswith(ext) else filepath + ext
                )
